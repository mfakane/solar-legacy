...
namespace Lunar
{
	...
	public class OAuthAuthorization : IDisposable
	{
		...
		public OAuthAuthorization(AccountToken token)
			: this("CONSUMER_KEY", "CONSUMER_SECRET", token)
		{
		}
		...
		public Uri GetAuthorizationUri()
		{
			...
			parameters.Add("oauth_signature", CreateSignature("POST", TwitterUriBuilder.OAuth.RequestToken().AbsoluteUri, parameters));
		...
		public AccountToken Authenticate(string pin)
		{
			...
			parameters.Add("oauth_signature", CreateSignature("POST", TwitterUriBuilder.OAuth.RequestToken().AbsoluteUri, parameters, requestTokenSecret));
		...
		public string CreateParameters(string httpMethod, Uri uri, string query = null)
		{
			...
			oAuthParameters.Add("oauth_signature", CreateSignature(httpMethod, uriString, parameters));

			return "OAuth " + string.Join(", ", oAuthParameters.Select(_ => _.Key + "=" + ConvertUrlEncode(EscapeDataString(_.Value))));
		}
		...
		string CreateSignature(string httpMethod, string uri, Dictionary<string, string> parameters)
		{
		...
		string CreateSignature(string httpMethod, string uri, Dictionary<string, string> parameters, string tokenSecret)
		{
