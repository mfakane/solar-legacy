...
namespace Lunar
{
	...
	public class Status : ServiceObject, IEquatable<Status>, IEntry, IComparable<Status>
	{
		...
		/// <summary>
		/// エンティティから URL と展開済み URL を取得します。
		/// </summary>
		public virtual Dictionary<string, string> Urls
		{
			get
			{
				return urls ?? (urls = json.entities()
					? ((dynamic[])json.entities.urls).Select(_ => new
					{
						Url = (string)_.url,
						ExpandedUrl = _.expanded_url() ? (string)_.expanded_url : null,
					})
					.Concat((json.entities.media() ? (dynamic[])json.entities.media : new dynamic[0]).Select(_ => new
					{
						Url = (string)_.url,
						ExpandedUrl = _.media_url_https() ? (string)_.media_url_https : null,
					}))
					.Where(_ => _.ExpandedUrl != null)
					.ToLookup(_ => _.Url, _ => _.ExpandedUrl)
