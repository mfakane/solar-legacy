...
namespace Lunar
{
	...
	public class TwitterClient : IDisposable
	{
		...
		dynamic UploadDynamic(Uri uri, string method, object args)
		{
			...
			})
			{
				...
				if (this.IsAuthorized)
					client.Headers[HttpRequestHeader.Authorization] = this.Authorization.CreateParameters(method, uri, parameters);
		...
		dynamic DownloadDynamicInternal(Uri uri, OAuthAuthorization authorization)
		{
			...
			})
			{
				...
				if (authorization != null && authorization.Token != null)
					client.Headers[HttpRequestHeader.Authorization] = authorization.CreateParameters("GET", uri);
		...
		public class TwitterStatuses
		{
			...
			{
				var rt = client.DownloadDynamicInternal(TwitterUriBuilder.Search.Query(query, range.SinceID, range.MaxID, range.Count, range.Page), null);

				foreach (dynamic i in rt.results)
					if (i != null)
		...
		public class TwitterLists
		{
			...
			{
				return new List(client, client.UploadDynamic(TwitterUriBuilder.Lists.Create(), new
				{
					name = listName,
			...
			{
				return new List(client, client.UploadDynamic(TwitterUriBuilder.Lists.Update(), new
				{
					owner_id = client.Account.UserID.ToString(),
					slug = listName,
					name = newListName,
					mode = mode == ListMode.None ? null : mode == ListMode.Public ? "public" : "private",
					description,
			...
			{
				return new List(client, client.UploadDynamic(TwitterUriBuilder.Lists.Destroy(client.Account.UserID.ToString(), listName)));
			}
			...
			public List AddMember(string listName, string add)
			{
				...
				return new List(client, client.UploadDynamic(TwitterUriBuilder.Lists.MembersCreate(), new
				{
					owner_id = client.Account.UserID.ToString(),
					list_id = long.TryParse(listName, out list) ? list : 0,
					slug = list == 0 ? listName : null,
					user_id = long.TryParse(add, out user) ? user : 0,
					screen_name = user == 0 ? add : null,
				}));
			}
			...
			public List RemoveMember(string listName, string remove)
			{
				...
				return new List(client, client.UploadDynamic(TwitterUriBuilder.Lists.MembersDestroyAll(), new
				{
					owner_id = client.Account.UserID.ToString(),
					list_id = long.TryParse(listName, out list) ? list : 0,
					slug = list == 0 ? listName : null,
					user_id = long.TryParse(remove, out user) ? user : 0,
					screen_name = user == 0 ? remove : null,
				}));
			}
		}
		...
		public class TwitterFavorites
		{
			...
			/// <returns>お気に入りの一覧。</returns>
			public IEnumerable<Status> Index(StatusRange range)
			{
				return Index(client.Account.UserID, range);
			}
			...
			{
				return Index(id, new StatusRange());
			}
			...
			/// <param name="page">ページ。</param>
			/// <returns>お気に入りの一覧。</returns>
			public IEnumerable<Status> Index(UserID id, StatusRange range)
			{
				return Index(id.ToString(), range);
			}
			...
			{
				return Index(userName, new StatusRange());
			}
			...
			/// <param name="page">ページ。</param>
			/// <returns>お気に入りの一覧。</returns>
			public IEnumerable<Status> Index(string userName, StatusRange range)
			{
				foreach (dynamic i in client.DownloadDynamic(TwitterUriBuilder.Favorites.Index(userName, range.SinceID, range.MaxID, range.Count, range.Page), false))
					if (i != null)
						yield return client.StatusCache.SetStatus(new Status(client, i));
			}
