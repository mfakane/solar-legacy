...
namespace Lunar
{
	...
	public class TwitterStream : IDisposable
	{
		...
		void ThreadMain(object state)
		{
			...
			try
			{
				var uri = TwitterUriBuilder.Stream.User(this.Track, this.Follows);
				...
				using (var ns = wc.OpenRead(uri + (string.IsNullOrEmpty(uri.Query) ? "?" : "&") + auth.CreateParameters("GET", uri).Replace(", ", "&").Substring("OAuth ".Length)))
