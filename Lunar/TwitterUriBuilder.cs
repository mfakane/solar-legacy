...
#pragma warning disable 1591
		...
		public static class Lists
		{
			...
			{
				return Build("lists/create.json");
			}

			public static Uri Update()
			{
				return Build("lists/update.json");
			}
			...
			public static Uri Statuses(string id, string listID, StatusID sinceID, StatusID maxID, int count, int page)
			{
				...
				(
					...
					new
					{
						owner_id = long.TryParse(id, out user) ? user : 0,
						owner_screen_name = user == 0 ? id : null,
						list_id = long.TryParse(listID, out list) ? list : 0,
						slug = list == 0 ? listID : null,
			...
			public static Uri Index(string id, long cursor = default(long))
			{
				...
				(
					...
					new
					{
						user_id = long.TryParse(id, out user) ? user : 0,
						screen_name = user == 0 ? id : null,
			...
			public static Uri MembersCreate()
			{
				return Build("lists/members/create.json");
			}

			public static Uri MembersDestroyAll()
			{
				return Build("lists/members/destory_all.json");
			}
		...
		{
			public static Uri Index(string id, StatusID sinceID, StatusID maxID, int count, int page)
			{
				return Build
				(
					"favorites/" + id + ".json",
