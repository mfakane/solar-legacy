...
#pragma warning disable 1591
	...
	class LinkConverter : IValueConverter
	{
			...
			if (this.MenuItems)
				return text == null
					...
					: LinkRegex.Matches(text).Cast<Match>().Select(_ =>
					{
						...
						{
							var urls = converted.Urls;

							try
							{
								var uri = new Uri(_.Value);

								if (urls.ContainsKey(_.Value) && !string.IsNullOrEmpty(urls[_.Value]))
									uri = new Uri(urls[_.Value]);

								mi.CommandParameter = uri;
								ExpandUri(mi.Dispatcher, u => mi.Header = mi.ToolTip = u, uri);
							}
							catch
							{
			...
			if (matches.Count > 0)
			{
				foreach (Match i in matches)
				{
					...
					if (i.Groups["url"].Success && Uri.IsWellFormedUriString(i.Value.TrimEnd('%'), UriKind.Absolute))
					{
						...
						if (urls.ContainsKey(i.Value) && !string.IsNullOrEmpty(urls[i.Value]))
						{
							uri = new Uri(urls[i.Value]);
							Task.Factory.StartNew(() => link.Dispatcher.BeginInvoke((Action<string>)(__ =>
							{
								((Run)link.Inlines.FirstInline).Text = __;
								link.ToolTip = __;
							}), uri.ToString()));
						}

						ExpandUri(link.Dispatcher, _ => link.ToolTip = ((Run)link.Inlines.FirstInline).Text = _, uri);
					}
		...
		static void ExpandUri(Dispatcher dispatcher, Action<string> setLink, Uri uri)
		{
			...
			//            contains = ResolvedUriCache.ContainsKey(uri);
			...
			//            lock (ResolvedUriCache)
			//                value = ResolvedUriCache[uri];

			//            dispatcher.BeginInvoke(setLink, DispatcherPriority.Background, value);
			//        }
			//        else
			//        {
			...
			//                            lock (ResolvedUriCache)
			//                                ResolvedUriCache[uri] = rt;

			//                            dispatcher.BeginInvoke(setLink, DispatcherPriority.Background, rt);
