...
namespace Solar.Models
{
	...
	public class Client : NotifyObject, IDisposable
	{
		...
		Client()
		{
			...
			//Category.OnGetStatuses += (sender, e) =>
			//{
			//    foreach (var i in streams)
			//        using (i.Value.Follows.SusupendNotification(true, true))
			//        {
			//            i.Value.Follows.Clear();
			//            i.Value.Follows.AddRange(Category.GetInstances()
			//                                           .SelectMany(_ => _.Filter.Sources.OfType<UserFilterSource>())
			//                                           .Where(_ => _.Account == 0 || _.Account == i.Key.UserID)
			//                                           .Select(_ => _.UserID));
			//        }
			//};
		}
		...
		internal static ScriptEngine EnsureScriptEngine()
		{
			if (Engine == null)
			{
				...
				foreach (var j in AppDomain.CurrentDomain.GetAssemblies())
					try
					{
						if (j.GetName().Name != "mscorlib")
							Runtime.LoadAssembly(j);
					}
					catch (Exception ex)
					{
						App.Log(new Exception("Exception while loading " + j.FullName + ": " + ex.Message, ex));
					}
			}
			...
			return Task.Factory.StartNew(() =>
			{
				...
				try
				{
					using (new TwitterClient(account, this.StatusCache))
					{
						...
						//var newStatus = status.Favorited ? status.Unfavorite() : status.Favorite();

						//UpdateStatus(newStatus);

						//foreach (var i in this.Groups.SelectMany().Any(__ => __.Filter.Sources.OfType<FavoritesFilterSource>()))
						//    if (newStatus.Favorited && !i.Statuses.Contains(newStatus))
						//        i.Statuses.Insert(0, newStatus);
						//    else if (!newStatus.Favorited)
						//        i.Statuses.Remove(newStatus);
					}
				}
				catch (Exception ex)
				{
					...
					try
					{
						foreach (var i in this.Accounts.Reverse())
							using (var client = new TwitterClient(i, this.StatusCache))
							{
								...
								{
									this.RateLimits.Where(__ => __.Account == i).Freeze().Run(__ => this.RateLimits.Remove(__));
